...
static int master(int argc, char **argv) {
    ...
    xbt_assert(sscanf(argv[1],"%ld", &num_tasks) == 1, "Invalid number of tasks");    /* - Number of tasks      */
    xbt_assert(sscanf(argv[2],"%lf", &min_comp_size) == 1, "Invalid minimum computation size");    /* - Number of tasks      */
    xbt_assert(sscanf(argv[3],"%lf", &max_comp_size) == 1, "Invalid maximum computation size");    /* - Number of tasks      */
    xbt_assert(sscanf(argv[4],"%lf", &min_comm_size) == 1, "Invalid minimum communication size");    /* - Number of tasks      */
    xbt_assert(sscanf(argv[5],"%lf", &max_comm_size) == 1, "Invalid maximum communication size");    /* - Number of tasks      */
    xbt_assert(sscanf(argv[6],"%ld", &num_workers) == 1, "Invalid number of workers");    /* - Number of tasks      */

    XBT_INFO("Got %ld workers and %ld tasks to process", num_workers, num_tasks);

    auto master_mailbox    = sg4::Mailbox::by_name("master_mailbox");

    for (int i = 0; i < num_tasks; i++) { /* For each task to be executed: */
        /* Wait for a worker request */
        auto worker_id = *((long *)master_mailbox->get());
...
{
    long id;
    xbt_assert(argc == 2, "The worker expects exactly one argument");
    xbt_assert(sscanf(argv[1],"%lu", &id) == 1, "Invalid id");
...
void create_platform_file(std::string filepath,
    ...
    fprintf(pf, "    <prop id=\"watt_per_state\" value=\"100.0:120.0\" />\n");
    fprintf(pf, "    <prop id=\"watt_off\" value=\"10\" />\n");
    fprintf(pf, "  </host>\n");
	...
	double core_speed = double_randfrom(min_core_speed, max_core_speed);
        ...
        fprintf(pf, "    <prop id=\"watt_per_state\" value=\"100.0:93.33333333333333:200.0, 93.0:90.0:170.0, 90.0:90.0:150.0\" />\n");
        fprintf(pf, "    <prop id=\"watt_off\" value=\"10\" />\n");
        fprintf(pf, "  </host>\n");
    }
...
void create_deployment_file(std::string filepath,
    ...
    for (int i=0; i < num_hosts * num_cores_per_hosts; i++) {
